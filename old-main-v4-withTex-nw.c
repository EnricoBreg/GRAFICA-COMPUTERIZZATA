...
/**
 * 2 coordinates for texture mapping
 */
#define N_TEXTURE_COORD 2
...
/**
 * Local storage for bitmap file 
*/
#ifndef DEBUG_MODE
struct BitMapFile *image[6];
#else
struct BitMapFile *image = NULL;
#endif
/**
 * File bmp for textures
 */
#ifndef DEBUG_MODE
char *filename[6] = {
    "./textures/blue.bmp",
    ...
    "./textures/yellow.bmp"};
#else
char *filename = "./textures/launch.bmp";
#endif
 ...
 */
#ifndef DEBUG_MODE
static GLenum textureID[6];
#else
static GLenum textureID[1];
#endif

/**
 * inputKey, utilizzata per salvare la "mossa" che si vuole fare (i.e. rotazione)
...
typedef struct Vertex
{
    ...
    float textureCoords[N_TEXTURE_COORD]; // Texture coordinates
} Vertex;

GLuint indicesArray[N_FACES * N_VERTICES];
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    {
        image[texIndex] = readBMP(filename[texIndex]);
        if (image[texIndex] == NULL)
        {
        ...
        {
            printf("File %s has size %d x %d \n", filename[texIndex], image[texIndex]->sizeX, image[texIndex]->sizeY);
        }
    }
#else
    // read bitmap image
...
void init(void)
{
    ...
    for (unsigned int texIndex = 0; texIndex < 6; texIndex++)
    {
        ...
        glTexImage2D(
            GL_TEXTURE_2D, 0, GL_RGBA, image[texIndex]->sizeX, image[texIndex]->sizeY, 0,
            GL_RGB, GL_UNSIGNED_BYTE, image[texIndex]->data);
    ...
    for (vaoIndex = 0; vaoIndex < 27; vaoIndex++)
    {
        ...
        glTexCoordPointer(2, GL_FLOAT, sizeof(verticesArray[0]), (GLvoid *)(sizeof(verticesArray[0].textureCoords)));
    }
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        for (j = 0; j < 3; j++)
        {
            ...
            for (k = 0; k < 3; k++)
            {
                ...
                {
                    // Bind texture array for every face
#ifndef DEBUG_MODE
                    glBindTexture(GL_TEXTURE_2D, textureID[faceIndex]);
#else
                    glBindTexture(GL_TEXTURE_2D, textureID[0]);
#endif
                    //glDrawElements(GL_TRIANGLE_STRIP, 2, GL_UNSIGNED_INT, (GLvoid *)(faceIndex * N_TEXTURE_COORD * sizeof(GLuint)));
                    // Drawing the relative cube face
